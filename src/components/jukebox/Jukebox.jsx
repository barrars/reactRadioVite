...
export default function Jukebox({
  ...
  // create var  currentDownload in a useRef hook
  ...
  useEffect(() => {
    ...
    socket?.on("downloadingSongTitle", (data) => {
      setCurrentDownload(data)
      console.log("downloadingSongTitle", data);
    });
    return () => {
      socket?.off("song");
      socket?.off("click");
      socket?.off("eta");
      socket?.off("downloadingSongTitle");
    };
  }, [socket, songList]);
  ...
  return (
    <div className="content-center flex  text-center row-span-2">
      ...
      />
      <p>
        socket connected? {socketConnection} id = {socketId}
      </p>
    </div>
  );
}
